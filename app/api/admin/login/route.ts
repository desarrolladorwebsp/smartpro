...
function respondWithSession(email: string, role: AdminSessionRole) {
  const cookieValue = createSessionToken(email, role);
...
export async function POST(request: Request) {
  try {
    ...
    const user = await verifyExecutiveCredentials(email, password);

    if (!user) {
      return NextResponse.json({ error: "Credenciales inválidas." }, { status: 401 });
    }

    return respondWithSession(user.email, user.role);
  } catch (error) {
